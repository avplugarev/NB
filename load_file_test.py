...
class FileLoader:
    ...
    def load_file(path_to_file):
        ...
        goods_target_classes = []  # пустой список под категории для товаров
        ...
        for row in working_sheet.values:
            ...
            print('товар № {0}, добавлен с описанием {1}'.format((len(goods_description)), good))
            #print(goods_description.append(good))
    ...
    def text_transformation(value, row, good):
        ...
        chip = {'LOT': 'без_чипа', 'DMX': 'обувь', 'datamatrix': 'обувь', 'MEX': 'мех', 'обычный': 'без_чипа',
                'Обычный': 'без_чипа', 'FUR': 'мех', 'обычно': 'без_чипа'}
        ...
        # заменяем числовые значения в описание товара - значениями из справочника
            ...
            #return good + ' ' + str(value)
            # приводим значения слов к нормальной форме - пока отключенено
            #print(text_normalisation.filtr_by_word_form(value, good))
            return text_normalisation.filtr_by_word_form(value, good)
...
path2 = 'file_to_load/data_set_3.xlsx'
# path2 = 'teach_test_sample.xlsx'
#print(FileLoader.load_file(path2))
